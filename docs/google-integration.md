...
第一版採用 Google Apps Script Web App 作為前端與 Google Workspace 的橋接層。它能把班級資料同步到 Google Sheets、在 Drive 建立時間戳記 JSON 備份，並用 Google Docs 產生班級學習報告。部署後的本系統也可以直接嵌入 Google Sites。
...
- 「建立 Google Docs 報告」會在 Drive 教學資料夾產生班級摘要，內容把學業平均與獎勵點數分開呈現。
- Google Drive 檔案上傳 API 已包含在橋接器中；第一版前端仍以 IndexedDB 離線檔案庫為主，避免大檔經由 Apps Script 傳輸時觸發配額限制。
