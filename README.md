...
- 正向獎勵：六種集點行為說明、個人或多人快速加點、四個點數級距、獎品目錄與完整兌換流水帳；獎品可設庫存，兌換後進入待交付清單直到實際交給學生。
- 個人趨勢：點學生編號開啟面板，含成績逐次評量趨勢（附班級平均對照）、出席逐月統計、點數累積與觀察紀錄時間軸。
...
- 出席：月曆檢視查詢與補改歷史出席，補改留稽核紀錄；當月出席率、逐日趨勢、需關注名單與 CSV 匯出。
- 成績：評量新增／編輯／複製／刪除與權重檢查、缺考標記（未補分時以應考平均 −5 計算）、CSV 成績匯入含比對預覽與一次性復原。
- 學生管理：回收筒（軟刪除可還原）、停用（保留歷史不列入名單）與轉班（紀錄一起轉移並改用新班級編號）。
- 備份：完整 JSON 本機備份／還原、Google Drive 最新備份還原，以及每日自動備份（保留 30 份）。
...
71 項端對端與單元測試涵蓋十個頁面、舊資料遷移、學生管理、成績與匯入、出席、資源分年級與手機版面，針對建置後的正式產物執行。詳見 [自動化測試](docs/testing.md)。
